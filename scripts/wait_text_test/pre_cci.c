# 1 "c:\\test\\tests\\scripts\\wait_text_test\\\\combined_wait_text_test.c"
...
# 1 "c:\\test\\tests\\scripts\\wait_text_test\\\\combined_wait_text_test.c" 2
...
# 2 "c:\\test\\tests\\scripts\\wait_text_test\\\\combined_wait_text_test.c" 2
...
# 1 "C:\\Program Files (x86)\\HP\\LoadRunner\\include/lrrte.h" 1
 




# 1 "C:\\Program Files (x86)\\HP\\LoadRunner\\include/te_if.h" 1
...
typedef struct rte_client_data * RteHandle ;





# 6 "C:\\Program Files (x86)\\HP\\LoadRunner\\include/lrrte.h" 2
  ...
  int TE_find_text(const char *pattern, int col1, int row1, int col2, int row2, int *ret_col,
				 int *ret_row, char *ret_text);
  int TE_get_cursor_pos(int *col, int *row);
  char * TE_get_line_attribute(int col, int row, int width, char *attr);
  ...
  char * TE_sperror(void);
  int TE_type(const char *str );
  ...
  int TE_wait_text(const char *pattern, int timeout, ...);




  int TE_record_logfile(void);
...
# 3 "c:\\test\\tests\\scripts\\wait_text_test\\\\combined_wait_text_test.c" 2
...
Action1()
{
	...
	lr_end_transaction("TE_connect", 2);

	 
	
	 
	TE_wait_text(pattern, 30, 1, 1, 80, 24, &ret_col, &ret_row, 0);
	...
	lr_end_transaction("TE_find_text", 2);
...
lr_start_transaction("TE_disconnect");
	...
	lr_end_transaction("TE_disconnect", 2);

	
	return 0;
}
# 4 "c:\\test\\tests\\scripts\\wait_text_test\\\\combined_wait_text_test.c" 2
...
# 5 "c:\\test\\tests\\scripts\\wait_text_test\\\\combined_wait_text_test.c" 2
